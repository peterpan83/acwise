...
class PathSixS(PathCalculator):

    @classmethod
    def getUsrDefineAerosolType(cls):
        i = 1
        aero_coms = []
        for item in product(np.arange(0, 12, 1) * 0.1, repeat=4):
    ...
    def __loadLUTs(self):
        import h5py
        lut_paths = glob.glob(os.path.join(self.__data_shared_dir,self.sensor,'path*{}_{}_*.h5'.format(self.sensor_altitude,self.groud_altitude)))
        self.dic_values = {}
        lut_paths.sort()
        aero_names_all = self.getUsrDefineAerosolType()
        for i in tqdm(range(len(lut_paths)),desc='Loading path reflectance LUTs'):
            data = h5py.File(lut_paths[i], 'r')
            aero_type_index = int (os.path.splitext(os.path.split(lut_paths[i])[-1])[0].split('_')[-1])
            _logger.debug("Loading path LUT:{}".format(aero_type_index))
            if i == 0:
            ...
            rhop_lut = data['path_reflectance'][()].reshape(
                (self.taua550_lut.shape[0], self.solz_lut.shape[0], self.vewz_lut.shape[0], self.phi_lut.shape[0], self.wavelenth_lut.shape[0]))

            ### this rhop includes the part of the diffuse sky reflectance reflected by the air-water surface
            rhop_t_lut = data['path_t_reflectance'][()].reshape(
                (self.taua550_lut.shape[0], self.solz_lut.shape[0], self.vewz_lut.shape[0], self.phi_lut.shape[0], self.wavelenth_lut.shape[0]))
            ...
            tran_up_lut = data['trans_up'][()].reshape(
                (self.taua550_lut.shape[0], self.solz_lut.shape[0], self.vewz_lut.shape[0], self.phi_lut.shape[0], self.wavelenth_lut.shape[0]))[:,0,:,0,:]
            tran_down_lut = data['trans_down'][()].reshape(
                (self.taua550_lut.shape[0], self.solz_lut.shape[0], self.vewz_lut.shape[0], self.phi_lut.shape[0], self.wavelenth_lut.shape[0]))[:,:,0,0,:]

            self.dic_values[aero_type_index] = (rhop_lut,rhop_t_lut,albedo_lut,tran_up_lut,tran_down_lut)

            self.__aerosol_types.append(aero_type_index)
            _logger.debug("{},{}".format(aero_type_index,len(aero_names_all)))
    ...
    def __init__(self,sensor,**kwargs):
        super().__init__()
        _logger.info('Initializing path reflectane caculation based on 6SV simulation....')
    ...
    def cal_reflectance(self, iband:int, solz:np.ndarray, senz:np.ndarray, phi:np.ndarray, aerosol_type:int, taua:float):
        '''
        calculating path reflectance for a given aerosol type and AOT
        :param iband:
        :param solz:
        :param senz:
        :param phi:
        ...
        rhop = np.full_like(solz,np.nan,float)
        ...
        rhop_lut = self.dic_values[aerosol_type][0]
        ...
        values = interp.interpn(
            ...
            rhop_lut[:, :, :, :, iband], xi=xi)

        rhop[valid_mask] = values
        return rhop

    def cal_reflectance_t(self,iband:int, solz:np.ndarray, senz:np.ndarray, phi:np.ndarray, aerosol_type:int, taua:float):
        """
        calculating path reflectance that includes the part of sky reflectance reflected by air-water surface
         for a given aerosol type and AOT
        :param iband:
        :param solz:
        :param senz:
        :param phi:
    ...
    def cal_spherical_albedo(self,iband:int,aerosol_type:int,taua:float):
        ...
        :return:
        '''
        albedo_lut = self.dic_values[aerosol_type][2]
    ...
    def cal_trans_down(self,iband,solz,aerosol_type,taua):
        ...
        trans_down_lut = self.dic_values[aerosol_type][4]
    ...
    def cal_trans_up(self,iband,senz,aerosol_type,taua):
        ...
        trans_up_lut = self.dic_values[aerosol_type][3]
